...
bool BPTree::insert(keyType key, const valType& value)
{
    ...
    auto& header = node_tuple.node->nodePageHeader();
                                              ...
                                              const record_t& rec)
{
    ...
    leaf.node->range_assignment<record_t>(std::begin(temp),
                                        std::next(std::begin(temp), split));
    new_leaf_node->range_assignment<record_t>(std::next(std::begin(temp), split),
                                       std::end(temp));

    auto& new_header = new_leaf_node->nodePageHeader();
    auto& new_records = new_leaf_node->records();
    new_header.onePageNumber = header.onePageNumber;
    header.onePageNumber = new_node_id;
    new_header.parentPageNumber = header.parentPageNumber;

    CHECK(commit_node(leaf));
    CHECK(commit_node(new_node_id, *new_leaf_node));

    struct node_tuple right = { new_node_id, std::move(new_leaf_node) };
    return insert_into_parent(leaf, new_records[0].key, right);
}
                                              ...
                                              node_tuple& right)
{
    ...
    parent_node->range_assignment<internal_t>(std::begin(temp), std::next(std::begin(temp), split - 1));

    auto& new_header = new_node->nodePageHeader();

    new_header.onePageNumber = temp[split - 1].pageNumber;
    new_node->range_assignment<internal_t>(std::next(std::begin(temp), split), std::end(temp));
...
bool BPTree::remove_entry_from_node(node_tuple& target, keyType key)
{
    ...
    if (header.isLeaf)
    {
        int idx = target.node->satisfy_condition_first<record_t>([&](auto& now) {
            return now.key == key;
        });
    ...
    {
        int idx = target.node->satisfy_condition_first<internal_t>([&](auto& now) {
            return now.key == key;
        });
        CHECK_WITH_LOG(idx != static_cast<int>(header.numberOfKeys), false,
                       "invalid key: %ld", key);
                            ...
                            node_tuple& parent_tuple, int k_prime)
{
    ...
    else
    {
        ...
        nodeId_t pagenum;
        neighbor->emplace_back<internal_t>(k_prime,
